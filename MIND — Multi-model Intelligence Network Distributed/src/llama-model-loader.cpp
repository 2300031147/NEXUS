            ...
            if (ggml_backend_buffer_is_host(cur->buffer)) {
                ...
                if (upload_backend) {
                    ...
                    while (bytes_read < read_end - read_start) {
                        ...
                        size_t chunk_start = aligned_offset + bytes_read;
                        size_t chunk_end = chunk_start + read_size;
                        size_t data_start = offset;
                        size_t data_end = offset + n_size;
                        
                        size_t valid_start = std::max(chunk_start, data_start);
                        size_t valid_end = std::min(chunk_end, data_end);
                        
                        if (valid_start < valid_end) {
                            uintptr_t ptr_data = ptr_dest_aligned + (valid_start - chunk_start);
                            size_t data_to_copy = valid_end - valid_start;

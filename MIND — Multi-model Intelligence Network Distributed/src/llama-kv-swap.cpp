...
std::vector<llama_drive_info> llama_kv_get_available_drives() {
    ...
    for (int i = 0; i < mntcount; ++i) {
        ...
        llama_drive_info info;
        info.path = mntbuf[i].f_mntonname;
        info.device_node = mntbuf[i].f_mntfromname;
        info.fs_type = mntbuf[i].f_fstypename;
        info.total_bytes = (uint64_t) mntbuf[i].f_blocks * mntbuf[i].f_bsize;
...
size_t llama_kv_tiered_manager::calculate_block_bytes() const {
    ...
    for (uint32_t il = 0; il < n_layers; ++il) {
        ...
        if (use_turboquant) {
            ...
            if (v_trans) {
                total += k_row_size * block_size + n_embd_v_gqa * ggml_row_size(type_v, block_size);
            } else {
...
bool llama_kv_tiered_manager::evict_lru_block(
    ...
    if (is_uma_zero_copy()) {
        // UMA Logical chunking: transition without physically swapping to io_buffer
        if (cells) {
            for (uint32_t c = meta.cell_start; c < meta.cell_start + meta.id.n_tokens; ++c) {
                cells->set_warm(c, true);
                if (cells->seq_has(c, meta.id.seq_id)) {
                    cells->seq_rm(c, meta.id.seq_id);
                }
            }
        }
        meta.set_tier(CacheTierType::WARM);
...
int32_t llama_kv_tiered_manager::get_block_cell_start(const llama_kv_block_id & id, uint32_t stream_id) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    auto it = blocks.find(id);
    if (it != blocks.end() && it->second.stream_id == stream_id) {
        if (it->second.loc == llama_kv_block_loc::HOT_VRAM || 
            (it->second.loc == llama_kv_block_loc::WARM_RAM && is_uma_zero_copy())) {
...
void llama_kv_tiered_manager::touch_or_promote_block(const llama_kv_block_id & id, uint32_t stream_id, llama_kv_cells * cells) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    auto it = blocks.find(id);
    if (it != blocks.end() && it->second.stream_id == stream_id) {
        if (it->second.loc == llama_kv_block_loc::WARM_RAM && is_uma_zero_copy()) {
            it->second.set_tier(CacheTierType::HOT);
            it->second.access_count++;
            it->second.access_ts = ++current_ts;
            
            auto warm_it = warm_ram_map.find(it->second.id);
            if (warm_it != warm_ram_map.end()) {
                warm_ram_list.erase(warm_it->second);
                warm_ram_map.erase(warm_it);
            }
            if (used_ram_bytes >= block_bytes) used_ram_bytes -= block_bytes;
            else used_ram_bytes = 0;
            
            auto lru_it = lru_map.find(it->first);
            if (lru_it != lru_map.end()) lru_list.erase(lru_it->second);
            lru_list.push_front(it->first);
            lru_map[it->first] = lru_list.begin();
            ...
            for (auto & tier : tiers) {
                ...
                if (tier.type == CacheTierType::HOT && tier.unified) tier.used += block_bytes;
            }
            
            if (cells) {
                for (uint32_t c = 0; c < it->second.id.n_tokens; ++c) {
                    uint32_t cell_idx = it->second.cell_start + c;
                    if (!cells->seq_has(cell_idx, it->second.id.seq_id)) {
                        cells->seq_add(cell_idx, it->second.id.seq_id);
                    }
                    cells->set_warm(cell_idx, false);
                }
            }
        }
    }
...
int32_t llama_kv_tiered_manager::swap_in_block(
        llama_seq_id seq_id,
        llama_pos pos_start,
        uint32_t cell_start,
        const std::vector<ggml_tensor *> & k_layers,
    ...
    if (meta.loc == llama_kv_block_loc::WARM_RAM) {
        if (is_uma_zero_copy() && meta.ram_ptr == nullptr && meta.cell_start == cell_start) {
            ...
            if (cells) {
                for (uint32_t c = 0; c < meta.id.n_tokens; ++c) {
                    uint32_t cell_idx = meta.cell_start + c;
                    if (!cells->seq_has(cell_idx, meta.id.seq_id)) {
                        cells->seq_add(cell_idx, meta.id.seq_id);
                    }
                    cells->set_warm(cell_idx, false);
                }
            }

            return meta.cell_start;
        }
        ...
        if (use_turboquant) {
            ...
            if (v) {
                if (!v_trans) {
                    ...
                    const size_t v_stride = ggml_row_size(type_v, kv_size);
                    const size_t slice_size = ggml_row_size(type_v, meta.id.n_tokens);
                    std::vector<float> temp_f32(meta.id.n_tokens);
                    std::vector<uint8_t> temp_raw(slice_size);

                    for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
                        for (uint32_t t = 0; t < meta.id.n_tokens; ++t) {
                            temp_f32[t] = v_f32[t * n_embd_v_gqa + j];
                        }
                        
                        if (type_v == GGML_TYPE_F32) {
                            std::memcpy(temp_raw.data(), temp_f32.data(), slice_size);
                        } else if (type_v == GGML_TYPE_F16) {
                            ggml_fp32_to_fp16_row(temp_f32.data(), (ggml_fp16_t *) temp_raw.data(), temp_f32.size());
                        } else if (ggml_get_type_traits(type_v)->from_float_ref) {
                            ggml_get_type_traits(type_v)->from_float_ref(temp_f32.data(), temp_raw.data(), temp_f32.size());
                        }

                        const size_t v_offset = j * v_stride + ggml_row_size(type_v, eff_cell);
                        if (v_offset + slice_size <= ggml_nbytes(v)) {
                            ggml_backend_tensor_set(v, temp_raw.data(), v_offset, slice_size);
                        }
                    }
                }
            }
...
void llama_kv_tiered_manager::div_seq(llama_seq_id seq_id, llama_pos p0, llama_pos p1, int d) {
    ...
    for (auto it = blocks.begin(); it != blocks.end();) {
        ...
        // Any block overlapping the div range must be deleted
        if ((p1 < 0 && block_end > p0) || (it->first.pos_start < p1 && block_end > p0)) {
            if (it->second.loc == llama_kv_block_loc::COLD_SSD) {
                store->free_slot(it->second.swap_slot);
                dec_tier(CacheTierType::COLD);
...
void llama_kv_tiered_manager::cp_seq(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, uint32_t stream_id_dst, llama_pos p0, llama_pos p1) {
    ...
    for (auto it = blocks.begin(); it != blocks.end(); ++it) {
        ...
        if ((p1 < 0 || it->first.pos_start < p1) && block_end > p0) {

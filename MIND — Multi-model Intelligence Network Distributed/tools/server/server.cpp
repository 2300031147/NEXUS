...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    // Initialize Native SwarmCode Node
    static SwarmNode swarm_node("coder", model_name, params.port);
    swarm_node.start();
    ...
    // SwarmCode Endpoints
    ctx_http.get("/v1/cluster/peers", [&](const server_http_req & req) -> server_http_res_ptr {
        auto res = std::make_unique<server_http_res>();
        auto peers = swarm_node.get_active_peers();
        ...
        res->data = payload.dump();
        res->content_type = "application/json; charset=utf-8";
        return res;
    });

    ctx_http.post("/v1/agent/discuss", [&](const server_http_req & req) -> server_http_res_ptr {
        auto res = std::make_unique<server_http_res>();
        json response = {
            {"status", "success"},
            {"response", "[Native Coder Node - " + model_name + "] Swarm Logic execution."}
        };
